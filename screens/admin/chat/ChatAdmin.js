...
const ChatAdmin = ({ route, navigation }) => {
    ...
    (async () => {
      try {
        const res = await fetch(
          `http://192.168.18.24/mascotas-api/endpoints/chat/obtener_mensajes_por_chatId.php?id_chat=${id_chat}`
        );
  ...
  const enviarImagen = async (uri) => {
    ...
    try {
      const res = await fetch(
        'http://192.168.18.24/mascotas-api/endpoints/chat/guardar_mensaje_admin.php',
        {
          method: 'POST',
  ...
  const handleImagenSeleccionada = (uri) => {
    // Mostrar vista previa en lugar de enviar inmediatamente
    setImgUri(uri);
  };
