...
const ChatUsuario = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setIdUsuario(num);
      try {
        const res = await fetch(`http://192.168.18.24/mascotas-api/endpoints/chat/obtener_mensajes_por_chat.php?id_usuario=${num}`);
  ...
  const uploadImage = async uri => {
    ...
    try {
      const res = await fetch(
        'http://192.168.18.24/mascotas-api/endpoints/chat/subir_imagen_chat.php',
  ...
  const enviarMensaje = async () => {
    ...
    try {
      const res = await fetch(
        'http://192.168.18.24/mascotas-api/endpoints/chat/guardar_mensaje_usuario.php',
...
const styles = StyleSheet.create({
  ...
  previewContainer: { flexDirection: 'row', alignItems: 'center', padding: 8, backgroundColor: '#fff', marginHorizontal: 8, borderRadius: 8, marginBottom: 4 },
  previewImage: { width: 80, height: 80, borderRadius: 8, marginRight: 8 },
  removePreview: { position: 'absolute', top: 4, right: 4, backgroundColor: '#ddd', borderRadius: 10, padding: 2 },
  removePreviewText: { fontSize: 12, color: '#333' },
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', marginHorizontal: 8, borderRadius: 8, paddingHorizontal: 8 },
  input: { flex: 1, height: 40 },
  boton: { backgroundColor: '#0084ff', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, marginLeft: 8 },
